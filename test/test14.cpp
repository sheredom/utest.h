...
UTEST(cpp14, ASSERT_TRUE) { ASSERT_TRUE(1); }

UTEST(cpp14, ASSERT_FALSE) { ASSERT_FALSE(0); }

UTEST(cpp14, ASSERT_EQ) { ASSERT_EQ(1, 1); }

UTEST(cpp14, ASSERT_NE) { ASSERT_NE(1, 2); }

UTEST(cpp14, ASSERT_LT) { ASSERT_LT(1, 2); }

UTEST(cpp14, ASSERT_LE) {
  ASSERT_LE(1, 1);
  ASSERT_LE(1, 2);
}

UTEST(cpp14, ASSERT_GT) { ASSERT_GT(2, 1); }

UTEST(cpp14, ASSERT_GE) {
  ASSERT_GE(1, 1);
  ASSERT_GE(2, 1);
}

UTEST(cpp14, ASSERT_MEMEQ) { 
  const char a1[4] = {1, 2, 3, 4};
  const char a2[4] = {1, 2, 3, 4}; 
  ASSERT_MEMEQ(a1, a2, 4);
}

UTEST(cpp14, EXPECT_MEMEQ) { 
  const char a1[4] = {1, 2, 3, 4};
  const char a2[4] = {1, 2, 3, 4}; 
  EXPECT_MEMEQ(a1, a2, 4);
}

UTEST(cpp14, ASSERT_STREQ) { ASSERT_STREQ("foo", "foo"); }

UTEST(cpp14, ASSERT_STRNE) { ASSERT_STRNE("foo", "bar"); }

UTEST(cpp14, ASSERT_STRNEQ) { ASSERT_STRNEQ("foo", "foobar", strlen("foo")); }

UTEST(cpp14, ASSERT_STRNNE) { ASSERT_STRNNE("foo", "barfoo", strlen("foo")); }

UTEST(cpp14, EXPECT_TRUE) { EXPECT_TRUE(1); }

UTEST(cpp14, EXPECT_FALSE) { EXPECT_FALSE(0); }

UTEST(cpp14, EXPECT_EQ) { EXPECT_EQ(1, 1); }

UTEST(cpp14, EXPECT_NE) { EXPECT_NE(1, 2); }

UTEST(cpp14, EXPECT_LT) { EXPECT_LT(1, 2); }

UTEST(cpp14, EXPECT_LE) {
  EXPECT_LE(1, 1);
  EXPECT_LE(1, 2);
}

UTEST(cpp14, EXPECT_GT) { EXPECT_GT(2, 1); }

UTEST(cpp14, EXPECT_GE) {
  EXPECT_GE(1, 1);
  EXPECT_GE(2, 1);
}

UTEST(cpp14, EXPECT_STREQ) { EXPECT_STREQ("foo", "foo"); }

UTEST(cpp14, EXPECT_STRNE) { EXPECT_STRNE("foo", "bar"); }

UTEST(cpp14, EXPECT_STRNEQ) { EXPECT_STRNEQ("foo", "foobar", strlen("foo")); }

UTEST(cpp14, EXPECT_STRNNE) { EXPECT_STRNNE("foo", "barfoo", strlen("foo")); }

UTEST(cpp14, no_double_eval) {
  int i = 0;
  ASSERT_EQ(i++, 0);
  ASSERT_EQ(i, 1);
}
...
UTEST_I(MyTestI, cpp14_1, 2) {
  ASSERT_GT(2u, utest_fixture->bar);
  utest_fixture->foo = 13;
}

UTEST_I(MyTestI, cpp14_2, 128) {
